...
% Module die een net geparste ply omzet in een move met een van en naar een coordinate
...
ply_move(ply(PieceName, Takes, co(R, C), Disambiguation, no_promotion), Check, move(From, (R, C)), state(StartBoard, Info, Color)) :-
    ...
    check(Check, NextState),
    get(From, StartBoard, FromPiece), name(FromPiece, PieceName),   % Piece must have same name as in the ply
